...
columns = ["Date", "Time", "Booking ID", "Booking Status", "Customer ID", "Vehicle Type", "Pickup Location",
           ...
           "Customer Demand", "Vehicles Breakdown", "Other Issue"]
data = []
...
for _ in range(1000000):
    ...
    booking_status = \
    random.choices(["Success", "Cancelled by Customer", "Cancelled by Driver"], weights=[62, 10, 15], k=1)[0]
    vtat = random.randint(5, 15) if booking_status == "Success" else None
    ctat = random.randint(5, 10) if booking_status == "Success" else None
    reason_for_cancellation = random.choice(["Driver is not moving", "Driver asked to cancel", "Change of plans",
                                             "Wrong Address"]) if booking_status == "Cancelled by Customer" else None
    cancelled_rides_by_driver = random.choice(
        ["Personal Issue", "Car related issue"]) if booking_status == "Cancelled by Driver" else None
    ...
    incomplete_rides_reason = random.choice(
        ["Customer Demand", "Vehicles Breakdown", "Other Issue"]) if incomplete_rides == 1 else None
    ...
    data.append(
        [date.strftime("%Y-%m-%d"), time, booking_id, booking_status, customer_id, vehicle_type, pickup_location,
         ...
         vehicles_breakdown, other_issue])
